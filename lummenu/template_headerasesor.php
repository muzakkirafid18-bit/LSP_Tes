...
// Cek level akses sesuai $user_level yang di-set halaman
$_allowed_levels = ($user_level === 'asesor') ? ['asesor'] : ['lsp'];
if (!in_array($_SESSION['level'] ?? '', $_allowed_levels)) {
    echo "<!DOCTYPE html><html><head><meta charset='utf-8'>
    <link href='https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;600;700&display=swap' rel='stylesheet'>
    <link href='https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css' rel='stylesheet'>
    <style>body{font-family:'Plus Jakarta Sans',sans-serif;display:flex;align-items:center;justify-content:center;height:100vh;background:#F4F8FA;margin:0}</style></head><body>
    <div style='text-align:center;background:#fff;padding:40px;border-radius:14px;box-shadow:0 4px 32px rgba(15,42,58,.13)'>
    <i class='fas fa-ban' style='font-size:2rem;color:#EF4444;margin-bottom:16px;display:block'></i>
    <h3 style='color:#1A2E3B;margin-bottom:16px'>Anda Tidak Punya Hak Akses!</h3>
    <a href='../lsp_login.php' style='display:inline-block;padding:10px 24px;background:#3BBFBF;color:#fff;border-radius:8px;text-decoration:none;font-weight:600'>Kembali ke Login</a>
    </div></body></html>";
    exit;
}
...
<aside class="sidebar">
    <div class="sidebar-logo">
        <?php if ($user_level === 'asesor'): ?>
        <!-- Logo gambar untuk asesor -->
        <div style="
            width:50px; height:50px;
            background:#fff; border-radius:12px; padding:5px;
            display:flex; align-items:center; justify-content:center;
            box-shadow:0 0 10px rgba(0,0,0,.1); flex-shrink:0;
        ">
            <img src="../images/lsplogosmkn1.png"
                 style="width:100%;height:100%;object-fit:contain"
                 alt="LSP Logo">
        </div>
        <div class="logo-text">
            <strong style="color:#fff">LSP</strong>
            <span>SMKN 1 CIBINONG</span>
        </div>
        <?php else: ?>
        <!-- Logo teks untuk admin LSP -->
        <div class="logo-box">LSP</div>
        <div class="logo-text">
            <strong>LSP SMKN 1</strong>
            <span>CIBINONG</span>
        </div>
        <?php endif; ?>
    </div>
