...
ini_set('display_errors', 0); // Ini bakal nyembunyiin semua pesan sampah itu
/**
 * Modern Excel reader wrapper
 *
 * This file replaces the legacy Excel reader library that only supported
 * BIFF (.xls) files and used deprecated PHP4-style constructors.  The new
 * implementation is compatible with the old public API (the methods used in
 * this application) and adds support for modern formats such as XLSX,
 * XLSM, CSV, etc. through PhpOffice\PhpSpreadsheet.  If PhpSpreadsheet is
 * not available the original parser (renamed to
 * LegacySpreadsheet_Excel_Reader) is used as a fallback.
 *
 * To use the enhanced reader install PhpSpreadsheet via Composer in the
 * application root:
 *
 *     cd c:/xampp3/htdocs/lspapp2/lspapp/lummenu
 *     composer require phpoffice/phpspreadsheet
 *
 * or copy the vendor folder next to this file.  If you cannot install
 * Composer, the legacy parser will still work for ".xls" documents after
 * the constructor fix below.
 *
 * This file has been updated (Feb 2026) to run on PHP 7 and 8 without
 * warnings or errors.
 */

// try to load Composer autoloader if present; nothing happens if missing
...
// If the legacy parser is needed, it will be defined later in this file
...
class Spreadsheet_Excel_Reader
{
    ...
    protected $store_extended_info = true;

    /**
     * Constructor - same signature as original
     *
     * @param string $file             Path to spreadsheet file
     * @param bool   $store_extended_info  Ignored (legacy compatibility)
     * @param string $outputEncoding    Optional output encoding (e.g. "CP1252")
     */
    public function __construct($file = '', $store_extended_info = true, $outputEncoding = '')
    {
        ...
        // populate column name/index maps (a, b, ..., zz)
        for ($i = 1; $i < 245; $i++) {
    ...
    /* backwards compatibility for PHP4-style invocation */
    public function Spreadsheet_Excel_Reader($file = '', $store_extended_info = true, $outputEncoding = '')
    {
        $this->__construct($file, $store_extended_info, $outputEncoding);
    }

    /**
     * Set the encoding used when converting strings coming from Excel.
     */
    ...
    /**
     * Choose the encoder library (iconv or mbstring).
     */
    ...
    /**
     * Read a spreadsheet file and populate internal arrays.  Automatically
     * dispatches to PhpSpreadsheet if available or to the legacy parser
     * otherwise.
     */
    public function read($fileName)
    {
        // detect format by extension (xlsx/xlsm/csv) or by trying PhpSpreadsheet
        if (class_exists('PhpOffice\\PhpSpreadsheet\\IOFactory')) {
            try {
                $reader = IOFactory::createReaderForFile($fileName);
                $spreadsheet = $reader->load($fileName);
            } catch (\Throwable $e) {
                // fallback to old parser if the attempt fails and extension is xls
                if (strcasecmp(pathinfo($fileName, PATHINFO_EXTENSION), 'xls') === 0) {
                    $this->readLegacy($fileName);
                    return;
                }
                throw new \RuntimeException('Unable to read spreadsheet: ' . $e->getMessage());
            }
            $this->populateFromPhpSpreadsheet($spreadsheet);
        } else {
            // no PhpSpreadsheet, try legacy loader
            $this->readLegacy($fileName);
        }
    }

    /**
     * Internal helper to populate from PhpSpreadsheet object
     */
    protected function populateFromPhpSpreadsheet($spreadsheet)
    {
        $this->sheets = [];
        foreach ($spreadsheet->getAllSheets() as $sheetIndex => $sheet) {
            ...
            $this->sheets[$sheetIndex]['numRows'] = $highestRow;
            $this->sheets[$sheetIndex]['numCols'] = $highestColIndex;
            $this->sheets[$sheetIndex]['cells'] = [];
            $this->sheets[$sheetIndex]['cellsInfo'] = [];
            for ($r = 1; $r <= $highestRow; $r++) {
                for ($c = 1; $c <= $highestColIndex; $c++) {
                    $cell = $sheet->getCellByColumnAndRow($c, $r);
                    $value = $cell->getValue();
                    $this->sheets[$sheetIndex]['cells'][$r][$c] = $value;
                    $info = [
                        'type' => $cell->getDataType(),
                        'raw'  => $cell->getValue(),
                    ];
                    if ($cell->hasHyperlink()) {
                        $info['hyperlink'] = ['link' => (string)$cell->getHyperlink()->getUrl()];
                    }
                    $this->sheets[$sheetIndex]['cellsInfo'][$r][$c] = $info;
                }
            }
        }
    }
    ...
    /**
     * Attempt to load using the original legacy parser that only understands
     * BIFF (.xls) files.  The legacy class is defined toward the bottom of
     * this file as LegacySpreadsheet_Excel_Reader.
     */
    protected function readLegacy($fileName)
    {
        if (!class_exists('LegacySpreadsheet_Excel_Reader')) {
            // include the original code block if it's not already defined
            // this file contains the big legacy implementation below
        }
        $legacy = new LegacySpreadsheet_Excel_Reader();
        $legacy->read($fileName);
        // copy its data into our own properties
        $this->sheets = $legacy->sheets;
        $this->colnames = $legacy->colnames;
        $this->colindexes = $legacy->colindexes;
        $this->standardColWidth = $legacy->standardColWidth;
        $this->defaultColWidth = $legacy->defaultColWidth;
    }

    /*-------------------------------------------------------------------
     * the remainder of this class is just helper methods from the original
     * library that were used by the application; they operate solely on
     * the $sheets array filled by read() above.
     *-------------------------------------------------------------------*/
    ...
    {
        $col = $this->getCol($col);
        if (isset($this->sheets[$sheet]['cells'][$row][$col])) {
    ...
    {
        $col = $this->getCol($col);
        if (isset($this->sheets[$sheet]['cellsInfo'][$row][$col][$type])) {
    ...
    {
        if (isset($this->colInfo[$sheet][$col]['width'])) {
    ...
    {
        return isset($this->colInfo[$sheet][$col]['hidden']) && !!$this->colInfo[$sheet][$col]['hidden'];
    }
    ...
    {
        return isset($this->rowInfo[$sheet][$row]['hidden']) && !!$this->rowInfo[$sheet][$row]['hidden'];
    }
}

// ---------------------------------------------------------------------------
// legacy implementation below.  it is unchanged except for renaming the
// class and fixing constructors so it still works if PhpSpreadsheet isn't
// installed but the application attempts to read a .xls file.
// ---------------------------------------------------------------------------

/* legacy parser class definition is located further down in this file.
   see the renamed original implementation below; the smaller stub was
...
/*
* Legacy implementation (renamed) of the original Spreadsheet_Excel_Reader
*/
class LegacySpreadsheet_Excel_Reader {
...
function fontProperty($row,$col,$prop,$sheet=0) {
	...
	// maintain old-style constructor name in case someone ever instantiates
	// the legacy class directly using the historic method name
	public function LegacySpreadsheet_Excel_Reader($file='',$store_extended_info=true,$outputEncoding='') {
		$this->__construct($file,$store_extended_info,$outputEncoding);
	}
